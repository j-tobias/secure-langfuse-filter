...
1. **Privacy by default** — Content is redacted, users are anonymised, PII is stripped
2. **Non-invasive** — The `body` dict is never mutated; OpenWebUI is unaware of the filter
3. **Operational focus** — Collects token usage, response times, model info, and message structure
...
| [Langfuse Integration](docs/LANGFUSE-INTEGRATION.md) | How the filter maps OpenWebUI's request/response cycle to Langfuse traces, spans, and generations. Covers the SDK methods used, trace lifecycle, token usage extraction, memory management, and error handling. |
| [Privacy Model](docs/PRIVACY-MODEL.md) | Detailed breakdown of what is and isn't sent to Langfuse. Documents all five sanitisation layers, PII detection mechanisms, the threat model, and known limitations. |
...
├── session  = chat_id
├── tags     = ["open-webui", ...]
├── metadata = { chat_title, chat_tags, response_time_ms, model_id, ... }
│
├── Span: user_input      → redacted messages
├── Generation: llm_response → model, redacted I/O, token usage
│
├── Span: user_input      (message 2)
├── Generation: llm_response (message 2)
└── ...
```
...
|-------|---------|--------|
| `redact_content` | `True` | Replace message text with size summaries |
| `insert_tags` | `True` | Add `open-webui` + task name as trace tags |
| `debug` | `False` | Enable sanitised debug logging |
